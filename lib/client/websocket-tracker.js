...
class WebSocketTracker extends Tracker {
  ...
  announce (opts) {
    ...
    if (opts.event === 'stopped' || opts.event === 'completed') {
      ...
      this._generateTrickleOffers(numwant, offers => {
  ...
  _onAnnounceResponse (data) {
    ...
    if (data.offer && data.peer_id) {
      ...
      const { sdp, candidate } = data.offer
      const isTrickleSdp = SDP_TRICKLE_REGEX.test(sdp)
      if (sdp) {
        debug('creating peer (from remote offer)')
        peer = this._createPeer({ trickle: this.trickle })
        peer.id = common.binaryToHex(data.peer_id)

        const listener = peer.trickle ? 'on' : 'once'
        peer[listener]('signal', answer => {
          const params = {
            action: 'announce',
            info_hash: this.client._infoHashBinary,
            peer_id: this.client._peerIdBinary,
        ...
        // Save reference to trickling peer to add candidates later
        if (isTrickleSdp) {
    ...
    if (data.answer && data.peer_id) {
      ...
      if (peer) {
        ...
        const { sdp, candidate } = data.answer
        const isTrickleSdp = SDP_TRICKLE_REGEX.test(sdp)
  ...
  _generateTrickleOffers (numwant, offersCb, candidateCb) {
    const self = this
    const offers = []
    const candidates = []
    let hasSentOffers = false
    debug('generating %s trickle offers', numwant)

    for (let i = 0; i < numwant; ++i) {
        generateTrickleOffer()
    }
    checkOffersDone()

    function generateTrickleOffer () {
        const offerId = randombytes(20).toString('hex')
        const binaryOfferId = common.hexToBinary(offerId)
        debug('creating peer (from _generateTrickleOffers)')
        const opts = { initiator: true, trickle: true }
        const peer = self.peers[offerId] = self._createPeer(opts)
        peer.on('signal', offer => {
          const offerObj = { offer, offer_id: binaryOfferId }
          if (offer.sdp) {
            offers.push(offerObj)
            checkOffersDone()
          } else if (!hasSentOffers) {
            // Only send candidates after initial offers are sent
            candidates.push(offerObj)
          } else {
            candidateCb(offerObj)
          }
        })
        peer.trackerTimeout = setTimeout(() => {
          debug('tracker timeout: destroying peer')
    ...
    function checkOffersDone () {
      if (offers.length === numwant) {
        debug('generated %s offers', numwant)
        hasSentOffers = true
        offersCb(offers)
        candidates.forEach(candidateCb)
      }
    }
  }

  _setPeerTrackerTimeout (peer, offerId) {
    peer.trackerTimeout = setTimeout(() => {
        debug('tracker timeout: destroying peer')
        peer.trackerTimeout = null
        delete this.peers[offerId]
    ...
    // Allow time for trickle candidates to arrive before deleting the peer.
  _setPeerTrickleTimeout (peer, offerId) {
    clearTimeout(peer.trickleTimeout)
    peer.trickleTimeout = setTimeout(() => {
        if (this.destroyed) { return }
        debug('trickle timeout: destroying peer')
        delete this.peers[offerId]
    }, TRICKLE_TIMEOUT);
    if (peer.trickleTimeout.unref) peer.trickleTimeout.unref()
  }
