...
function Swarm (infoHash, server) {
  ...
  // maps announce_id to a set of peer_ids to prevent sending
  // duplicate offers to the same peer within one announce event.
  self.announceGroups = new LRU({
      max: 1000,
      maxAge: 60 * 1000 * 5 // 5 min
  })
...
Swarm.prototype._getPeersByOfferId = function (params, ownPeerId) {
    const peers = []
